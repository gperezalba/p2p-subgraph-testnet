...
export function createOffer(event: NewOffer): void {
    ...
    offer.price = event.params.buyAmount.div(event.params.sellAmount).times(getOneEther());
...
export function createOfferCommodity(event: NewOfferCommodity): void {
    ...
    if (token.category == BigInt.fromI32(1)) {
        ...
        offer.price_per_brute_weight = event.params.buyAmount.div(gold.weight_brute as BigInt).times(getOneEther());
        offer.price_per_fine_weight = event.params.buyAmount.div(gold.weight_fine as BigInt).times(getOneEther());
    }
...
export function updateOffer(event: UpdateOffer): void {
    ...
    offer.sellAmount = event.params.sellAmount;
    offer.buyAmount = event.params.buyAmount;
    offer.price = event.params.buyAmount.div(event.params.sellAmount as BigInt).times(getOneEther());
